...
function keydown(event){var target=event.target;var modifier=event.ctrlKey||event.altKey||event.metaKey||(event.shiftKey&&event.keyCode!==key.spacebar);target.blur()
target.focus()
...
function addEvent(type,fn){window.addEventListener(type,fn,{passive:!1})}
function removeEvent(type,fn){window.removeEventListener(type,fn,{passive:!1})}
